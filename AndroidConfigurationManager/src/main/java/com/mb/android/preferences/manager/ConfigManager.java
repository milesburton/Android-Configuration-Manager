...
public class ConfigManager implements OnSharedPreferenceChangeListener {
	...
	public void saveConfig(Config config) {
		...
		editor.putString(CONFIG_ID_LIST_KEY,
				StringCollectionUtils.join(newIdList, ","));
		ConfigSerialiser configSerialiser = new SharedPreferenceConfigSerialiser(
				editor);
	...
	private void processConfig(String configId) {
		...
		ConfigDeserialiser configSerialiser = new SharedPrefenceConfigDeserialiser(
				preferences);
		String type = configSerialiser.getType(configId);

		if (!isInvalidConfig(type)) {
			ConfigProcessorStrategy configProcessor = configProcessorStragegyList
					.get(type);
	...
	public void addConfigProcessorStrategy(
			ConfigProcessorStrategy configProcessorStrategy) {
		this.configProcessorStragegyList.put(
				configProcessorStrategy.getSupportedConfigType(),
				configProcessorStrategy);
	}
