	...
	public void serialise(final Config config) {
		...
		ConfigValueAction putEditorAction = new ConfigValueAction() {
			...
			public void process(Field field, ConfigMetadata fieldMetadata) {
				String key = keyGenerator.createKey(config.getId(),
						fieldMetadata.id());
				Object value = getValueForField(config, field);

				SharedPreferenceConfigSerialiser.this.putEditorValue(editor,
						key, value);
			}

		};

		editor.putString(keyGenerator.createTypeKey(config.getId()),
				config.getType());
